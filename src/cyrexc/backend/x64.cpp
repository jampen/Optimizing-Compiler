...
struct AllocationStrategy
{
    ...
    AllocationStrategy(const std::string& strategy)
    {
        ...
        {
            case 'p':
                lifetime = X64::ValueLifetime::Persistent;
                break;
            case 't':
                lifetime = X64::ValueLifetime::Temporary;
                break;
            case 's':
                lifetime = X64::ValueLifetime::Scratch;
                break;
            case 'x':
                break;
            default:
...
static const std::unordered_map<Opcode, AllocationStrategy> alloc_strategy = {
    {Opcode::Alloc, {"p"}},     {Opcode::Const, {"sn"}},
    {Opcode::Store, {"xnn"}},   {Opcode::Load, {"tn"}},
    {Opcode::Add, {"tnn"}},     {Opcode::Sub, {"tnn"}},
    {Opcode::Lesser, {"tnn"}},  {Opcode::LesserOrEqual, {"tnn"}},
    {Opcode::Greater, {"tnn"}}, {Opcode::GreaterOrEqual, {"tnn"}},
    {Opcode::Equal, {"tnn"}},   {Opcode::NotEqual, {"tnn"}},
    {Opcode::And, {"tnn"}},     {Opcode::Or, {"tnn"}},
    {Opcode::Xor, {"tnn"}},     {Opcode::Label, {"xn"}},
    {Opcode::Branch, {"xnnn"}}, {Opcode::Jump, {"xn"}},
    {Opcode::Return, {"xn"}},
};
...
void X64::instruction(std::vector<MC>& mc, const Inst& inst)
{
    ...
    const auto result = [&]()
    {
        return operand(inst.result);
    };
    const auto inst_operand = [&](int index)
    {
        return operand(inst.operands[index]);
    };
    const auto src = [&]()
    {
        return inst_operand(0);
    };
    const auto lhs = [&]()
    {
        return inst_operand(0);
    ...
    switch (inst.opcode)
    {
        ...
        break;
        case Store:
            push_mc(MC::mov(inst_operand(0), inst_operand(1)));
            break;
        case Load:
        ...
        case Xor:
            break;
        case Label:
            push_mc(MC::label(inst.operands[0]));
            break;
        case Branch:
...
X64::Operand X64::constant(const ValueId constant_value_id)
{
    ...
    return std::visit([&](auto&& v) { return Operand::make_imm((int64_t)v, constant_value_id); },
    ...
    for (const auto& ins : mc)
    {
        switch (ins.op)
        {
            ...
            case Mov:
                ...
                {
                    ts << format("\tmov {} {}, {}\n",
                                 type_size(ir.get_value_by_id(ins.dst->value_id).type).str(),
                                 emit(*ins.dst),
                                 emit(*ins.src));
                    break;
                }
