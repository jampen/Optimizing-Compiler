...
static constexpr CompileTimeComparison conditional_jump_to_comparison(const X64::MC& mc)
{
    ...
    switch (mc.op)
    {
        case Jl:
            return res::Lesser;
        case Jle:
            return res::LesserOrEqual;
        case Jg:
            return res::Greater;
        case Jge:
            return res::GreaterOrEqual;
        case Je:
            return res::Equal;
        case Jne:
            return res::NotEqual;
...
static constexpr bool compile_time_binary_compare(auto l, auto r, CompileTimeComparison cmp)
{
    ...
    for (auto it = mc.begin(); it != mc.end();)
    {
        ...
        if (false)
            if (remaining(5))
            {
                ...
                if (a.op == Mov && a.src->is_imm() && b.op == Mov && b.src->is_imm() && c.op == Mov && c.dst->is_rax() &&
                    *c.src == *a.dst && d.op == Cmp && *d.lhs == *c.dst && *d.rhs == *b.dst && e.is_conditional_jump())
                {
        ...
        if (remaining(4))
        {
            ...
            if (a.op == Mov && a.dst->is_rax() && a.src->is_imm(1) && b.op == Test && b.lhs->is_rax() &&
                b.rhs->is_rax() && c.is_conditional_jump() && d.is_conditional_jump() && d.op == c.negated_jump().op)
            {
                auto dst = c.dst;
                it = mc.erase(it, it + 3);
        ...
        if (remaining(4))
        {
            ...
            if (a.op == Xor && a.dst->is_rax() && a.src->is_rax() && b.op == Test && b.lhs->is_rax() &&
                b.rhs->is_rax() && c.is_conditional_jump() && d.is_conditional_jump() && d.op == c.negated_jump().op)
            {
        ...
        if (remaining(2))
        {
            ...
            if (a.op == Mov && b.op == Mov && *a.src == *b.src && a.dst->is_rax())
            {
        ...
        if (remaining(3))
        {
            ...
            if (a.op == Mov && b.is_binary_math_operation() && c.op == Mov && *a.dst == *b.dst && *a.dst == *c.src &&
                *a.src == *c.dst)
            {
        ...
        {
            auto& b = it[1];
            if (a.op == Mov && b.is_binary_math_operation() && a.dst->is_reg() && *a.dst == *b.src)
            {
        ...
        {
            auto& b = it[1];
            if (a.op == Xor && a.dst->is_rax() && b.src->is_rax() && b.op == Mov && *b.src == *a.dst)
            {
        ...
        if (remaining(1))
        {
            ...
            if (a.op == Xor && *a.src == *a.dst && ir.literal_exists(a.src->value_id) && b.op == Cmp &&
        ...
        if (remaining(3))
        {
            auto b = it[1];
            auto c = it[2];
            if (a.op == Xor && a.src == a.dst && b.op == Cmp && b.lhs == a.src && b.rhs->is_imm() && b.rhs->imm == 0 &&
                c.op == Je)
            {
        ...
        {
            auto b = it[1];
            if (a.op == Xor && a.dst == a.src && b.op == Mov && b.dst == a.src && b.src->is_reg())
            {
                it = mc.erase(it);
                changed = true;
                continue;
        ...
        // mov XXX, 0
        // -> xor XXX, XXX
        if (a.op == Mov && a.src->is_imm() && a.src->imm == 0 && !a.dst->is_mem())
        {
        ...
        {
            return ins.op == Mov && ins.dst->is_mem() && ins.src->is_imm() && ins.src->imm == 0;
        };

        const auto is_self_xor = [&](const MC& mc)
        {
            return a.op == Xor && a.src->is_reg() && a.dst->is_reg() && a.src->reg == a.dst->reg;
        };
        ...
        if (remaining(6))
        {
            ...
            if (a.op == Cmp && b.is_setxx() && c.op == MovZx && d.op == Test && e.is_conditional_jump() &&
        ...
        if (remaining(5))
        {
            ...
            if (a.op == Cmp && b.is_setxx() && c.op == MovZx && d.op == Test && e.is_conditional_jump())
            {
                ...
                MC folds[2]{a, b.setxx_to_jumpxx(*e.dst)};
        ...
        if (remaining(4))
        {
            ...
            if (a.op == Mov && a.dst->is_rax() && a.src->is_imm() && cmp_mc.op == Cmp && cmp_mc.lhs->is_rax() &&
                cmp_mc.rhs->is_imm() && jmp_if_true.is_conditional_jump() && jmp_if_false.is_conditional_jump())
            {
                const auto l = a.src->imm;
        ...
        if (remaining(1))
        {
            ...
            if (a.op == Mov && b.op == Mov && a.src->is_imm() && a.dst->is_reg() && *b.dst == *a.src)
            {
        ...
        {
            auto b = it[1];
            if (a.op == Mov && a.dst->is_rax() && b.op == Cmp && b.lhs->is_rax())
            {
                auto old_src = a.src;
                it = mc.erase(it);
                it->lhs = old_src;
        ...
        if (remaining(4))
        {
            ...
            if (a.op == Cmp && b.is_conditional_jump() && c.op == b.negated_jump().op && d.op == Label &&
                *d.lbl == b.dst->imm)
            {
