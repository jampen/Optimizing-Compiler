...
public:
    ...
    static constexpr int num_qword_regs = 14;
    ...
    // System dependent!
    static constexpr std::array volatile_regs = {Reg::rax, Reg::rcx, Reg::rdx, Reg::r8, Reg::r9, Reg::r10, Reg::r11};
    static constexpr std::array callee_saved_regs = {Reg::rbx, Reg::r12, Reg::r13, Reg::r14, Reg::r15};

    static constexpr Reg to_largest_reg(const Reg reg)
    {
#define X(q, d, w, b) \
    case Reg::q:      \
    case Reg::d:      \
...
#undef X
    ...
    static constexpr bool is_reg_callee_saved(const Reg reg)
    {
...
#undef X
    ...
    static constexpr Reg same_reg_of_diff_size(const Reg reg, const RegSize size)
    {
    ...
    struct Operand
    {
        ...
        static constexpr Operand make_reg(const Reg r, const ValueId vid)
        {
            return {Kind::Reg, {.reg = r}, vid};
        }

        static constexpr Operand make_mem(const std::size_t stack, const ValueId vid)
        {
            return {Kind::Mem, {.stack = stack}, vid};
        }

        static constexpr Operand make_imm(const std::int64_t imm, const ValueId vid = NoValue)
        {
    ...
    static constexpr X64::Operand reg(const X64::Reg r, const ValueId value_id = NoValue)
    {
    ...
    struct MC
    {
        ...
        static constexpr MC mov(const Operand& dst, const Operand& src)
        {
            return MC{.op = Opcode::Mov, .dst = dst, .src = src};
        }

        static constexpr MC movzx(const Operand& dst, const Operand& src)
        {
            return MC{.op = Opcode::MovZx, .dst = dst, .src = src};
        }

        static constexpr MC push(const Operand& src)
        {
            return MC{.op = Opcode::Push, .src = src};
        }

        static constexpr MC pop(const Operand& src)
        {
            return MC{.op = Opcode::Pop, .src = src};
        }

        // Maths
        static constexpr MC add(const Operand& dst, const Operand& src)
        {
            return MC{.op = Opcode::Add, .dst = dst, .src = src};
        }

        static constexpr MC sub(const Operand& dst, const Operand& src)
        {
            return MC{.op = Opcode::Sub, .dst = dst, .src = src};
        }

        static constexpr MC inc(const Operand& src)
        {
            return MC{.op = Opcode::Inc, .src = src};
        }

        static constexpr MC dec(const Operand& src)
        {
            return MC{.op = Opcode::Dec, .src = src};
        }

        // Logic
        static constexpr MC l_and(const Operand& dst, const Operand& src)
        {
            return MC{.op = Opcode::And, .dst = dst, .src = src};
        }

        static constexpr MC l_or(const Operand& dst, const Operand& src)
        {
            return MC{.op = Opcode::Or, .dst = dst, .src = src};
        }

        static constexpr MC l_xor(const Operand& dst, const Operand& src)
        {
            return MC{.op = Opcode::Xor, .dst = dst, .src = src};
        }

        // Comparisons
        static constexpr MC cmp(const Operand& lhs, const Operand& rhs)
        {
            return MC{.op = Opcode::Cmp, .lhs = lhs, .rhs = rhs};
        }

        static constexpr MC test(const Operand& lhs, const Operand& rhs)
        {
            return MC{.op = Opcode::Test, .lhs = lhs, .rhs = rhs};
        }

        static constexpr MC setl(const Operand& dst)
        {
            return MC{.op = Opcode::Setl, .dst = dst};
        }

        static constexpr MC setle(const Operand& dst)
        {
            return MC{.op = Opcode::Setle, .dst = dst};
        }

        static constexpr MC setg(const Operand& dst)
        {
            return MC{.op = Opcode::Setg, .dst = dst};
        }

        static constexpr MC setge(const Operand& dst)
        {
            return MC{.op = Opcode::Setge, .dst = dst};
        }

        static constexpr MC sete(const Operand& dst)
        {
            return MC{.op = Opcode::Sete, .dst = dst};
        }

        static constexpr MC setne(const Operand& dst)
        {
            return MC{.op = Opcode::Setne, .dst = dst};
        }

        // Branches
        static constexpr MC jmp(const Operand& dst)
        {
            return MC{.op = Opcode::Jmp, .dst = dst};
        }

        static constexpr MC jz(const Operand& dst)
        {
            return MC{.op = Opcode::Jz, .dst = dst};
        }

        static constexpr MC jnz(const Operand& dst)
        {
            return MC{.op = Opcode::Jnz, .dst = dst};
        }

        static constexpr MC jl(const Operand& dst)
        {
            return MC{.op = Opcode::Jl, .dst = dst};
        }

        static constexpr MC jle(const Operand& dst)
        {
            return MC{.op = Opcode::Jle, .dst = dst};
        }

        static constexpr MC jg(const Operand& dst)
        {
            return MC{.op = Opcode::Jg, .dst = dst};
        }

        static constexpr MC jge(const Operand& dst)
        {
            return MC{.op = Opcode::Jge, .dst = dst};
        }

        static constexpr MC je(const Operand& dst)
        {
            return MC{.op = Opcode::Je, .dst = dst};
        }

        static constexpr MC jne(const Operand& dst)
        {
            return MC{.op = Opcode::Jne, .dst = dst};
        }

        // Returns
        static constexpr MC ret()
        {
            return MC{.op = Opcode::Ret};
        }

        static constexpr MC ret(const Operand& src)
        {
            return MC{.op = Opcode::Ret, .src = src};
        }

        static constexpr MC label(const int l)
        {
            return MC{.op = Opcode::Label, .lbl = l};
        }

        static constexpr MC nop()
        {
