...
bool Parser::expect(const Token::Type type, const std::string& error_message)
{
    ...
    if (!same)
        push_error(std::string(error_message));
    return same;
}
...
AST::Ptr Parser::parse_expr()
{
    ...
    {
        return make_ast(AST::LiteralExpr{.type = AST::Type{.name = "byte",
                                                           .qualifiers = {AST::Type::Qualifier{
...
AST::Ptr Parser::parse_tuple(const int size_limit)
{
    ...
    while (!check(Token::Type::RightSqBracket))
    {
        ...
        if (check(Token::Type::RightSqBracket))
            break;
    ...
    if (check(Token::Type::Assign))
    {
        ...
        return make_ast(AST::TupleAssignExpr{.tup_left = std::move(tuple), .tup_right = std::move(rhs)});
    }
...
AST::Ptr Parser::parse_if(Context context)
{
    ...
    if (check(Token::Type::LeftBrace))
    {
        ...
        return make_ast(AST::IfStmt{.condition = std::move(condition),
    ...
    if (!expect(Token::Type::Else, "expected else after if-expression"))
        return nullptr;
    next();
    ...
    return make_ast(
        AST::IfExpr{.condition = std::move(condition), .then_expr = std::move(then_expr), .else_expr = std::move(else_expr)});
}
...
AST::Ptr Parser::parse_while(Context context)
{
    ...
    if (check(Token::Type::LeftBrace))
    {
        ...
        if (check(Token::Type::Then))
        {
            ...
            return make_ast(
                AST::WhileExpr{.condition = std::move(condition), .block = std::move(block), .returns = std::move(expr)});
        }
    ...
    if (check(Token::Type::Do))
    {
        ...
        if (check(Token::Type::Then))
        {
            ...
            if (!expr)
                return nullptr;
            return make_ast(
                AST::WhileExpr{.condition = std::move(condition), .block = std::move(stmt), .returns = std::move(expr)});
        }
...
AST::Ptr Parser::parse_identifier()
{
    ...
    if (check(Token::Type::Assign))
    {
        ...
        if (!expr)
            return nullptr;
        return make_ast(AST::AssignExpr{.left = std::move(ident), .expr = std::move(expr)});
    }
...
{
    auto num = make_ast(AST::LiteralExpr{.type = AST::Type{.name = "int", .qualifiers = {}}, .value = next().text});
...
AST::Ptr Parser::parse_binary(AST::Ptr&& left)
{
    const AST::BinaryExpr::Kind kind = [&]()
    {
        ...
        {
            case Token::Type::Lesser:
                return AST::BinaryExpr::Kind::CmpLesser;
            case Token::Type::LesserOrEqual:
                return AST::BinaryExpr::Kind::CmpLesserOrEqual;
            case Token::Type::Greater:
                return AST::BinaryExpr::Kind::CmpGreater;
            case Token::Type::GreaterOrEqual:
                return AST::BinaryExpr::Kind::CmpGreaterOrEqual;
            case Token::Type::Equal:
                return AST::BinaryExpr::Kind::CmpEqual;
            case Token::Type::NotEqual:
                return AST::BinaryExpr::Kind::CmpNotEqual;
            case Token::Type::And:
                return AST::BinaryExpr::Kind::CmpAnd;
            case Token::Type::Or:
                return AST::BinaryExpr::Kind::CmpOr;
            case Token::Type::Xor:
                return AST::BinaryExpr::Kind::CmpXor;
            case Token::Type::Plus:
                return AST::BinaryExpr::Kind::Add;
            case Token::Type::Minus:
                return AST::BinaryExpr::Kind::Sub;
        }
    }();
    ...
    return make_ast(AST::BinaryExpr{.left = std::move(left), .right = std::move(expr), .kind = kind});
}
...
AST::Type Parser::parse_type()
{
    ...
    while (!is_at_end())
    {
        ...
        {
            // pointer
            type.qualifiers.push_back(AST::Type::Qualifier{.kind = AST::Type::Qualifier::Kind::Pointer});
            next();
        }
        else if (check(Token::Type::LeftSqBracket))
        {
            ...
            if (!expect(Token::Type::RightSqBracket, "expected ]"))
                return {};
            next();
            type.qualifiers.push_back(AST::Type::Qualifier{.kind = AST::Type::Qualifier::Kind::Array, .array_length = len});
        }
        else
        {
            break;
